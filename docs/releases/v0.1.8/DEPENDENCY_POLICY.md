...
- Add crossbeam-channel 0.5.17 (MIT OR Apache-2.0) for bounded, cancellation-aware
  channels. No prohibited dependency is introduced.
- Vendor the existing OpenH264 0.9.8 Rust wrapper (BSD-2-Clause) to configure
  nonzero decoder threads before Initialize. Zero leaves the decoder's unthreaded
  default untouched. Codec math is unchanged; unstable decoder threading remains
  outside the normal release build.
...
- The project itself retains the supplied BUSL-1.1 identifier. An exact
  package-and-version exception applies only to `tenzor-pipe =0.1.6`. BUSL is
