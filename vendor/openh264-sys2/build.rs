...
mod source_build {
    ...
    fn try_compile_nasm(target: &Target, cc_build_command: &mut Build, root: &str) {
        ...
        // TenzorPipe patch: upstream's build/x86-common.mk defines HAVE_AVX2 for x86 assembly
        // builds; this crate did not, so the AVX2 IDCT and luma motion-compensation routines
        // were compiled out. Selection is still by runtime CPUID, so non-AVX2 CPUs keep SSE paths.
        nasm_build = nasm_build.define("HAVE_AVX2", None);

        // Run `nasm` and store result.
        // TenzorPipe patch: a silent C fallback roughly halves decode speed, so fail the build
        // instead. OPENH264_NO_ASM (above) or OPENH264_ALLOW_C_FALLBACK opt out explicitly.
        let object_files = match nasm_build
            .files(glob_import(root, &config.asm_extension, &config.asm_exclude))
            .compile_objects()
        {
            Ok(objects) => objects,
            Err(e) if std::env::var("OPENH264_ALLOW_C_FALLBACK").is_ok() => {
                println!("cargo:warning=OpenH264 NASM compilation failed ({e}); using C fallback");
                return;
            }
            Err(e) => panic!(
                "OpenH264 NASM compilation failed: {e}. Install NASM >= 2.13, or set \
                 OPENH264_ALLOW_C_FALLBACK=1 to accept a much slower C-only decoder."
            ),
        };
        ...
        println!("openh264-sys2: x86 assembly enabled with HAVE_AVX2 ({} objects)", object_files.len());
