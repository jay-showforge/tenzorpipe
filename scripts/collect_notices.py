...
for p in sorted(meta['packages'],key=lambda p:p['name']):
    if p['id'] in meta['workspace_members']:continue  # project crates: see LICENSE
    ...
    if not files and dest.is_dir():
        # Crates without a packaged license file keep a curated notice (see the file for its source).
