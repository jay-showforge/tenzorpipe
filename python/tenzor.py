...
file does not concatenate/copy the entire dataset into one giant tensor.
"""
...
class TenzorDataset:
    ...
    def _tensor_from_fixed_list(column, rows: int, shape):
        ...
        return torch.from_numpy(values).view(rows, *shape)

    def get_batch(self, batch_index: int):
        rb = self.reader.get_batch(batch_index)
        out = {
            "timestamp_ms": torch.from_numpy(rb.column("timestamp_ms").to_numpy(zero_copy_only=True)),
        ...
        if self.has_video and "video_timestamp_ms" in rb.schema.names:
            out["video_timestamp_ms"] = torch.from_numpy(rb.column("video_timestamp_ms").to_numpy(zero_copy_only=True))
        if "audio_valid_frames" in rb.schema.names:
            out["audio_valid_frames"] = torch.from_numpy(rb.column("audio_valid_frames").to_numpy(zero_copy_only=True))
        if self.has_video:
